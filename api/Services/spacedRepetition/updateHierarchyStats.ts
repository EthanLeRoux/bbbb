...
import { calculateWeightedScore, TestAttempt } from './calculateWeightedScore';
import { calculateRecallQuality } from './calculateRecallQuality';
import { calculateRetentionStrength } from './calculateRetentionStrength';
...
export interface EntityStats {
  entityType: 'material' | 'section' | 'domain';
  entityId: string;
  userId: string;
...
export interface HierarchyUpdateInput {
  ...
  currentStats?: {
    material?: EntityStats;
    section?: EntityStats;
    domain?: EntityStats;
  };
}

export interface HierarchyUpdateResult {
  updatedStats: {
    material: EntityStats;
    section: EntityStats;
    domain: EntityStats;
 ...
 * 
 * @param input - Hierarchy update parameters
 * @returns Updated statistics for all hierarchy levels
 */
export async function updateHierarchyStats(input: HierarchyUpdateInput): Promise<HierarchyUpdateResult> {
 ...
 */
export function updateMaterialStats(input: HierarchyUpdateInput): EntityStats {
 ...
 */
export async function updateSectionStats(input: HierarchyUpdateInput, materialStats: EntityStats): Promise<EntityStats> {
 ...
 */
export async function updateDomainStats(input: HierarchyUpdateInput, sectionStats: EntityStats): Promise<EntityStats> {
  ...
  return {
    ...
    parentId: undefined // Domains have no parent
  };
}
 ...
 */
export function aggregateChildStats(childStats: EntityStats[], parentType: 'section' | 'domain'): {
  avgScore: number;
  weightedScore: number;
  reviewCount: number;
  streak: number;
 ...
 */
export function identifyWeakAreas(materialStats: EntityStats, sectionStats: EntityStats, domainStats: EntityStats): string[] {
 ...
 */
export function calculateReviewRecommendations(materialStats: EntityStats, sectionStats: EntityStats, domainStats: EntityStats): {
  material: Date;
  section: Date;
  domain: Date;
} {
 ...
 */
export function calculateHierarchyImpact(materialStats: EntityStats, sectionStats: EntityStats, domainStats: EntityStats): {
  materialPriority: number;
  sectionPriority: number;
  domainPriority: number;
} {
...
export async function getChildEntityStats(userId: string, childType: 'material' | 'section', parentId: string): Promise<EntityStats[]> {
  // This would query Firestore for child entity stats
  // Implementation depends on your data structure
  return [];
}

export function getHistoricalAttempts(userId: string, entityType: 'material' | 'section' | 'domain', entityId: string): TestAttempt[] {
  // This would query Firestore for historical test attempts
  // Implementation depends on your data structure
  return [];
}

export function calculateDaysSince(date1: Date, date2: Date): number {
