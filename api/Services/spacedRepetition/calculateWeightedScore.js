 ...
 * 
 * @param {Array} attempts - Array of test attempts with scores and completion dates
 * @param {Date} referenceDate - Reference date for decay calculation (default: now)
 * @returns {Object} WeightedScoreResult with weighted score and trend analysis
 * 
 * Decay Formula:
 * decay = e^(-daysAgo / 30)
 ...
 */
function calculateWeightedScore(attempts, referenceDate = new Date()) {
  // Input validation
 ...
 * 
 * @param {number} daysAgo - Number of days since the attempt
 * @returns {number} Decay factor (0-1, where 1 is most recent)
 */
function calculateRecencyDecay(daysAgo) {
 ...
 */
function calculateDaysAgo(date, referenceDate = new Date()) {
 ...
 */
function analyzeRecentTrend(attempts, referenceDate = new Date()) {
 ...
 */
function calculateWeightedScoreConfidence(attempts) {
 ...
 */
function validateWeightedScoreResult(result) {
  return (
...
module.exports = {
  ...
  getRecencyWeight,
  calculateWeightedScoreConfidence,
  validateWeightedScoreResult
};
