...
const FOCUS_FACTORS = [
  { key: 'lowScore',       label: 'Low avg score',    weight: 0.35, icon: '📉' },
  { key: 'frequentErrors', label: 'Frequent errors',  weight: 0.20, icon: '❌' },
  { key: 'recentDecline',  label: 'Recent decline',   weight: 0.15, icon: '↘' },
  { key: 'slowImprovement',label: 'Slow improvement', weight: 0.10, icon: '🐢' },
  { key: 'inactivity',     label: 'Inactivity',       weight: 0.10, icon: '⏳' },
  { key: 'difficulty',     label: 'Difficulty factor',weight: 0.10, icon: '🔥' },
];
...
function computeFocusScore(topic, questions) {
  ...
  const sorted = [...qs].sort((a, b) => (dateOf(a.date) || 0) - (dateOf(b.date) || 0));
  const all = sorted.map((q) => q.score);
  const avg = average(all);

  // Low score factor (0–1, higher = worse)
  const lowScore = clamp(1 - avg / 100, 0, 1);

  // Frequent errors factor
  const errorRate = all.filter((s) => s < 60).length / all.length;
  const frequentErrors = errorRate;

  // Recent decline: compare last-third vs first-third
  const third = Math.max(1, Math.floor(all.length / 3));
  const early = average(all.slice(0, third));
  const late  = average(all.slice(-third));
  const recentDecline = clamp((early - late) / 100, 0, 1); // positive means declined

  // Slow improvement: slope of regression
  const slope = all.length > 2
    ? (() => {
        const n = all.length;
        const xs = all.map((_, i) => i);
        const xMean = (n - 1) / 2;
        const num = xs.reduce((s, x, i) => s + (x - xMean) * (all[i] - avg), 0);
        const den = xs.reduce((s, x) => s + (x - xMean) ** 2, 0);
        return den ? num / den : 0;
      })()
    : 0;
  const slowImprovement = clamp(1 - clamp(slope / 2, 0, 1), 0, 1);

  // Inactivity: days since last attempt
  const lastDate = dateOf(sorted[sorted.length - 1]?.date);
  const daysSince = lastDate ? (Date.now() - lastDate.getTime()) / 86400000 : 999;
  const inactivity = clamp(daysSince / 30, 0, 1);

  // Difficulty: proxy via average time (we don't have explicit difficulty, use score variance)
  const variance = average(all.map((s) => (s - avg) ** 2));
  const difficulty = clamp(variance / 2500, 0, 1);

  const raw = { lowScore, frequentErrors, recentDecline, slowImprovement, inactivity, difficulty };
  const total = FOCUS_FACTORS.reduce((s, f) => s + f.weight * raw[f.key], 0);

  return { topic, score: clamp(total * 100, 0, 100), factors: raw, avgScore: avg, questionCount: qs.length, daysSince };
}
